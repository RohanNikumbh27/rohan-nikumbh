...
<form on:submit|preventDefault={() => {}}>
  ...
  >
    ...
    >
      <span class="flex items-center transition-all opacity-1">
        <span
          class="text-sm font-semibold whitespace-nowrap truncate text-white mx-auto"
        >
          Search
        </span>
      </span>
  ...
  {#if $SearchQ}
    <div class="text-black text-left font-[500] mb-5">
      Showing {filteredResults.length} result{filteredResults.length === 1
        ? ""
        : "s"} for "{$SearchQ}"
    </div>
    <div
      ...
      {#if filteredResults.length > 0}
        {#each filteredResults as result, i}
          <a href={result.link} class="px-1" on:click={toggle}>
            <div
              class="flex gap-3 items-center cursor-pointer hover:translate-x-1 transition-all"
            >
