<script>
  ...
  // Utility for detecting outside clicks
...
>
  ...
  >
    ...
    <svg
      class="w-5 h-5 text-gray-500 transform transition-transform duration-300 {isOpen
        ? 'rotate-180'
        : ''}"
      fill="none"
      stroke="currentColor"
  ...
  {#if isOpen}
    ...
    >
      <div
        class="flex h-[40px] justify-between items-center px-4 py-2 border-b border-[#0000001A]"
      >
        <span class="text-gray-600"
          >{heading ? heading : "Select " + label}</span
        >
