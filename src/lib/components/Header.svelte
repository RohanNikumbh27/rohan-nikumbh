...
>
  <div class="px-4">
      ...
      <button class="" on:click={handleSeachBarButton}>
        ...
        {#if !showNavbarSmall}
          ...
          <img
            src="\hamburger.svg"
            alt="hamburger"
            class="h- w-7 inline-block md:hidden"
          />
        {:else}
          <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
        ...
        {#if !showSearchComp}
          ...
          <img
            ...
            in:blur={{ duration: 300, delay: 100 }}
          />
        {:else}
          <!-- svelte-ignore a11y-no-noninteractive-element-interactions -->
          <img
            src="\closeiconround.svg"
            alt="close"
            class="h-6 hidden md:inline-block"
            in:blur={{ duration: 300, delay: 100 }}
          />
    ...
    <div
      class="px-3 transition-all sm:hidden hidden {showSearchComp
        ? 'md:block'
        : 'md:hidden'}"
    >
    ...
    <div
      class="md:hidden h-svh flex justify-center items-center {showNavbarSmall
        ? 'block'
        : 'hidden'}"
    >
      <div class="px-3 transition-all">
        <div
          class="sm:flex sm:flex-col md:flex-row flex-wrap sm:justify-center md:gap-5"
        >
          {#if $SearchQ == ""}
            {#each NavOptionsArr as NavOption, i}
              <a
